...
#define SD_READER_MODEL "Adafruit SPI Flash SD Card - XTSD 2 GB"
#define SD_SPI spi1
...
{
    printf("[SD] Startup test begins for %s on SPI1: MISO=GP%u CS=GP%u SCK=GP%u MOSI=GP%u\n",
           SD_READER_MODEL,
           (unsigned)SD_PIN_MISO,
           (unsigned)SD_PIN_CS,
           (unsigned)SD_PIN_SCK,
           (unsigned)SD_PIN_MOSI);
